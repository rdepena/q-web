...
commands: `fechVariables`somethingElse

fetchVariables: {
  send[`fetchVariables; {(`type`name)!(x 1;x 0)} each flip (v;type each value each v:value "\\v");x`id];
 }

fetchData: { //fetchData
  send[`fetchData; value x`data; x`id];
 }

fetchAsString: { //fetchAsString
  send[`fetchAsString; .Q.s value x`data; x`id];
 }

send: {
  message: (`cmd`data`id)!(x;y;z);
  neg[.z.w] .j.j message;
 }
