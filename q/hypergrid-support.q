...
fetchVariables: {
  send[`fetchVariables; {(`type`name)!(x 1;x 0)} each flip (v;type each value each v:value "\\v")];
 }

fetchData: { //fetchData
  send[`fetchData; value x];
 }

fetchAsString: { //fetchAsString
  send[`fetchAsString; .Q.s value x];
 }

send: {
  message: (`cmd`data)!(x;y);
  neg[.z.w] .j.j message;
 }
